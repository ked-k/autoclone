...
class PendingSamplesReportComponent extends Component
{
    use WithPagination;
    ...
    {
        $samples = Sample::targetSearch($this->search)->select('*')->whereNotIn('status', ['Tests Done'])->where('sample_is_for', 'Testing')
            ->where('creator_lab', auth()->user()->laboratory_id)->with(['participant', 'participant.facility', 'sampleType:id,type', 'study:id,name', 'sampleReception'])
    ...
    public function updatedSampleFacilityId()
    {
    ...
    public function render()
    {
        ...
        $data['samples']     = $this->filterSamples()->orderBy($this->orderBy, $this->orderAsc ? 'asc' : 'desc')
            ->paginate($this->perPage);

        return view('livewire.lab.reports.pending-samples-report-component', $data);
    }
}
